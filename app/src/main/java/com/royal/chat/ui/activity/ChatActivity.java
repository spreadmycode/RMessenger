...
import com.royal.chat.ui.adapter.AttachmentPreviewAdapter;
import com.royal.chat.ui.adapter.ChatAdapter;
...
import com.royal.chat.utils.imagepick.ImagePickHelper;
import com.royal.chat.utils.imagepick.OnImagePickedListener;
...
import com.royal.chat.utils.qb.VerboseQbChatConnectionListener;
...
public class ChatActivity extends BaseActivity implements OnImagePickedListener, OnAudioPickedListener, QBMessageStatusListener, DialogsManager.ManagingDialogsCallbacks, GetImageFileListener {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == SystemPermissionHelper.PERMISSIONS_FOR_SAVE_FILE_AUDIO_REQUEST && grantResults[0] != -1) {
            openAudioPicker();
        }
    }
    ...
    public void onImagePicked(int requestCode, File file) {
        switch (requestCode) {
            case REQUEST_CODE_ATTACHMENT:
                attachmentPreviewAdapter.add(file, QBAttachment.IMAGE_TYPE);
                break;
        }
    }
    ...
    @Override
    public void onAudioPicked(int requestCode, File file) {
        switch (requestCode) {
            case REQUEST_CODE_ATTACHMENT:
                attachmentPreviewAdapter.add(file, QBAttachment.AUDIO_TYPE);
                break;
        }
    }

    @Override
    public void onAudioPickError(int requestCode, Exception e) {
        showErrorSnackbar(0, e, null);
    }

    @Override
    public void onAudioPickClosed(int requestCode) {
    ...
    private void showAttachmentsMenu(View view) {
        PopupMenu attachmentsMenu = new PopupMenu(ChatActivity.this, view);
        attachmentsMenu.getMenuInflater().inflate(R.menu.activity_select_attachment, attachmentsMenu.getMenu());
        attachmentsMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            @Override
    ...
    private void openAudioPicker() {
        SystemPermissionHelper permissionHelper = new SystemPermissionHelper(this);
        if (permissionHelper.isSaveAudioPermissionGranted()) {
            new AudioPickHelper().pickAnAudio(this, REQUEST_CODE_ATTACHMENT);
        } else {
            permissionHelper.requestPermissionsForSaveFileAudio();
        }
    }
